...
`mw_sochi_ru`. (360°-панорама, сверхширокая панорама и 2 star-trails в прогон не
включались — структурно нерешаемы.)

**Ретрай с подсказками масштаба** (`degwidth 8–120°`, `downsample 2`) **не вытащил ни
одного** — включая `flickr_torchbearer_ladia`. Вывод: проблема не в поиске масштаба, а в
природе кадров (сшивка панорам ломает единую пинхол-проекцию; ландшафт/засветка/обработка
топят детекцию). Поэтому 13 таких кадров перетегованы в манифесте как **`track: scene`** —
это материал для **отдельного трека scene-локализации** (детекция полосы МП / pattern
matching, либо tile-and-solve), а не провалы солвера. Подробности — в отдельном
треке scene-локализации (вне scope plate-solve eval в этом репозитории).
